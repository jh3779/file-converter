...
def xlsx_to_csv(src: Path, tmpdir: Path) -> Path:
    ...
    # 범위 밖(의도적): 여기서 "첫 번째"는 워크북 내부 저장 순서 기준이라
    # 숨김 시트도 포함한다 — 실사용에서 맨 앞 시트가 숨김 메타데이터고
    # 사용자가 보는 첫 탭이 사실 두 번째인 경우까지는 보장하지 않는다.
    ws = wb.worksheets[0]
...
def csv_to_json(src: Path, tmpdir: Path) -> Path:
    ...
    if not rows:
        ...
        # 버린다 — csv.DictReader의 restkey 관례를 따라 별도 키에 리스트로
        # 보존한다(불규칙 CSV에서의 데이터 손실 방지). 원본 헤더에 이미
        # "_extra"가 있으면 그 컬럼 값을 덮어쓰게 되므로, 헤더와 겹치지
        # 않을 때까지 접두 언더스코어를 늘려 충돌 없는 키를 고른다.
        extra_key = "_extra"
