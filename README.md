...
- [x] **PDF→DOCX 밑줄 감지**(DEC-055, 변환 품질 개선 5순위) — 폰트 이름 휴리스틱으로는 원천적으로 판별 불가능한 밑줄을, 텍스트 바로 아래의 벡터 선(DEC-054가 이미 뽑아둔 도형 추출 재사용) 위치로 추정. 밑줄로 쓰인 선은 표 테두리 도형으로 중복 렌더링되지 않도록 제외. **검증 중 발견한 잔여 리스크**: `pdf_to_docx`의 절대 위치 프레임(DEC-037)이 실제 LibreOffice 렌더링에서 글자 대부분이 잘려 보이는 사전 존재 결함을 발견 — 밑줄과 무관하게 재현되는 별개 문제라 이번엔 범위 밖으로 두고 후속 과제로 분리(DEC-055 결정 로그 참고)
