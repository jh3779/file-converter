...
- **3D 모델**: OBJ/STL/PLY/GLB/GLTF 상호 변환 (형태·정점 색상 보존, STL은 포맷 자체에 색상·재질이 없어 STL로 변환 시 색이 사라짐 — DEC-050). **FBX(Autodesk)는 읽기 전용**으로 위 5개 포맷으로만 단방향 변환 가능(FBX로 내보내기는 미지원, 순수 Python 자체 파서 — DEC-069). 형태(정점·면)와 좌표축(Z-up→Y-up 등)만 옮기는 형태 위주 변환이라, 오브젝트 위치·회전·크기 이동(로컬 변환)과 단위 배율은 반영되지 않아 로컬 변환이 항등(이동·회전·스케일 없음)인 메시만 정확하고, 로컬 변환이 걸려 있거나 원점에서 벗어난 오브젝트·여러 오브젝트가 있는 씬은 위치·회전·크기가 달라질 수 있음. 머티리얼·색상·텍스처는 아예 읽지 않으며, 오목(non-convex) 다각형은 fan triangulation 특성상 삼각형 분할이 부정확할 수 있음(변환 전 고지)
...
## 실행 방법 (개발)
의존성은 [uv](https://docs.astral.sh/uv/)로 관리한다(`pyproject.toml`+`uv.lock`) — 설치는 [uv 설치 가이드](https://docs.astral.sh/uv/getting-started/installation/) 참고.
```bash
...
- [x] **표 셀 안 문자 서식(굵게/기울임/밑줄/크기/색상) 보존**(DEC-051, 변환 품질 개선 1순위) — DOCX↔HWP·DOCX↔HWPX 표 셀 텍스트도 문단과 완전히 같은 표현으로 통일해 서식을 그대로 왕복시킨다. DEC-027/038(문단 서식)·DEC-035/049(표 병합)가 이미 검증해둔 함수를 표 셀에 그대로 재사용해 새로운 위험 없이 구현
...
- [x] **표 셀 안 문단 정렬(가운데/오른쪽 등) 보존**(DEC-053, 변환 품질 개선 3순위) — DOCX↔HWP·DOCX↔HWPX 표 셀도 최상위 문단과 같은 `align` 필드를 갖게 통일했다(DEC-051이 셀 서식에 적용한 것과 같은 원칙). DEC-040/049(문단 정렬)가 이미 검증해둔 함수를 표 셀에 그대로 재사용해 새로운 위험 없이 구현. 셀 안에 문단이 여러 개면 첫 문단의 정렬만 대표값으로 쓴다(실사용에서 드문 경우)
- [x] **PDF→DOCX 이미지·표 테두리 반영**(DEC-054, 변환 품질 개선 4순위) — pdf_to_pptx(DEC-036)가 이미 구현해둔 이미지·도형 추출을 재사용하고, python-docx에 없는 자유배치 도형 API 대신 이미 검증된 절대 위치 메커니즘(`_set_frame_pr`, DEC-037)으로 빈 문단에 그림·테두리·채움을 얹는 방식으로 구현. 곡선은 다각형이 아닌 bounding box 사각형으로 더 단순하게 근사
...
- [x] **FBX(Autodesk) 읽기 전용 지원 신규**(DEC-069, 사용자 요청, OQ-007 해결) — 기성 라이브러리 `ufbx`가 세그폴트로 채택 불가해(spike/fbx/RESULT.md) 네이티브 확장이 전혀 없는 순수 Python 자체 파서를 직접 구현. FBX 7.x 바이너리(비압축·zlib 압축 둘 다)를 읽어 위 5개 3D 모델 포맷으로만 단방향 변환 가능(FBX로 내보내기는 미지원). 좌표축(Z-up→Y-up 등 GlobalSettings 기준)은 정규화하고 여러 지오메트리가 있으면 Connections로 실제 Model에 연결된 것만 골라 합친다. Model 노드의 위치/회전/크기 로컬 변환과 단위 배율은 적용하지 않아(변환 전 고지) 로컬 변환이 항등(identity)인 오브젝트(3D 프린팅용 단일 메시처럼 원점에 변환 없이 배치된 경우)만 정확하고, 회전·크기가 걸려 있거나 여러 오브젝트가 있는 씬은 위치가 달라질 수 있음 — FBX 6.x·ASCII FBX는 명확한 오류로 거부(크래시 아님)
