...
- **3D 모델**: OBJ/STL/PLY/GLB/GLTF 상호 변환 (형태·정점 색상 보존, STL은 포맷 자체에 색상·재질이 없어 STL로 변환 시 색이 사라짐 — DEC-050). **FBX(Autodesk)는 읽기 전용**으로 위 5개 포맷으로만 단방향 변환 가능(FBX로 내보내기는 미지원, 순수 Python 자체 파서 — DEC-069). 형태와 좌표축(Z-up→Y-up 등)은 정확히 반영되지만, 오브젝트 위치·회전·크기 이동(로컬 변환)과 단위 배율은 반영되지 않아 로컬 변환이 항등(이동·회전·스케일 없음)인 메시만 정확하고, 로컬 변환이 걸려 있거나 원점에서 벗어난 오브젝트·여러 오브젝트가 있는 씬은 위치·회전·크기가 달라질 수 있음(변환 전 고지)
