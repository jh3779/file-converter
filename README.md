...
- [x] **업데이트 확인 기능 추가, 옵트인·기본 꺼짐**(REQ-F-013, DEC-022 — 기획 초기부터 미결이던 OQ-002 해결): 설정에서 켜면 GitHub Releases에 버전 번호만 조회, 새 버전이 있으면 조용히 안내(클릭 시 릴리스 페이지로 이동). 자동 다운로드·설치 없음, 파일·경로·식별 정보 미전송 — 오프라인·요청 실패 시 조용히 무시
