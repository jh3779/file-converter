...
- [x] v0.3.3 실사용 테스트 피드백: HWP→PDF/DOCX 텍스트 보존 정상 확인(표만 의도한 대로 단순화). **지원 안 되는 파일의 ✕(제거) 버튼이 클릭에 반응하지 않던 버그 수정**(DEC-020) — 행 전체를 비활성화하면서 Qt의 부모-자식 활성 상태 전파로 제거 버튼 클릭까지 막혀 있었음
