...
- [x] v0.3.6 — **PDF→HWP 경로 추가**(DEC-023) — 확인해보니 DOCX→HWP·HWP→DOCX는 이미 있었고 PDF→HWP만 없어서 그 경로만 추가. PDF→DOCX와 같은 텍스트 기반 파이프라인(PDF는 표 구조를 안 담고 있어 별도 단순화 불필요). 영상→MP4 변환은 별도 조사 후 진행하기로 보류(FFmpeg 등 새 엔진·라이선스 재검토 필요, 기획 초기부터 MoSCoW "Could — 구조만 열어둠"으로 범위 밖)
