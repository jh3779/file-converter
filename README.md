...
> Windows 배포 (v0.3.1 프리릴리스) · macOS 개발 환경 · 사이드 프로젝트 · MVP 완성

## 다운로드

| 플랫폼 | 방법 |
|---|---|
| 🪟 **Windows** | **[v0.3.1 다운로드](https://github.com/jh3779/file-converter/releases/download/v0.3.1/FileConverter-Setup-latest.exe)** — 관리자 권한 불요, 인스톨러 실행 후 안내만 따라가면 끝 (v0.3c·DEC-013). 최신 버전·릴리스 노트는 [Releases](https://github.com/jh3779/file-converter/releases) 참고 |
...
- [x] **DOCX→HWP 신규 지원**(DEC-017, DEC-003 일부 번복) — hwplib로 HWP 파일을 새로 생성하는 사이드카(`JsonToHwp.java`) 추가. 문단 텍스트는 그대로 보존되고, 표는 hwplib에 신규 생성 도구가 없어 각 행을 텍스트 한 줄로 안전하게 단순화(내용 유실 없음)
