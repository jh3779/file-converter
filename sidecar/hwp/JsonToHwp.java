 ...
 *     "size":float,"color":"RRGGBB"}, ...]} | {"type":"p","text":str}(구버전
 *     호환, pdf_to_hwp가 여전히 이 형태를 씀 — 서식 없는 단일 run으로 취급) |
...
public class JsonToHwp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Object[]> items = new ArrayList<>(); // {"p", runs} or {"table", rows}
        for (Object o : blocks) {
            ...
            if ("table".equals(type)) {
                ...
                if (!runs.isEmpty()) items.add(new Object[]{"p", runs});
            }
        }
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if ("table".equals(kind)) {
                    ...
                    if (isFirst) {
                        ...
                        vpos = addTextParagraph(section, runs, isLast, vpos);
                    }
                }
            }
        }
    ...
    private static int addTextParagraph(Section section, List<Map<String, Object>> runs, boolean last, int startVpos) throws Exception {
    ...
    private static class JsonReader {
        ...
        Boolean readBoolean() {
            ...
            throw new RuntimeException("잘못된 boolean 리터럴 (pos=" + pos + ")");
        }

        Object readNull() {
            if (s.startsWith("null", pos)) { pos += 4; return null; }
            throw new RuntimeException("잘못된 null 리터럴 (pos=" + pos + ")");
        }

        /** 정수·소수 리터럴을 Double로 반환한다(run의 size 전용 — 이 스키마가
         * 필요로 하는 숫자는 그 정도뿐이라 별도 Long 분기는 두지 않는다). */
        Double readNumber() {
            int start = pos;
            if (s.charAt(pos) == '-') pos++;
