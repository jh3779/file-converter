...
이 5줄을 `python3 -c`로 8회 반복 실행 — **8/8 모두 `exit 139`(SIGSEGV)**. 스크립트 자체는 정상 출력 후 끝나지만, 인터프리터 종료 시점에 크래시. `del m; del scene`으로 명시적으로 먼저 지워도 동일하게 재현됨(코드 순서로 회피 불가).
...
### 결론: **실현 가능** — 검증한 FBX 7400/7500 바이너리 큐브 fixture(각 1개)에서 정점·폴리곤 데이터를 정확히 추출 성공
...
### 구현: `spike/fbx/spike_parser.py` (213줄, 표준 라이브러리만 — `struct`·`zlib`·`pathlib`)
...
- 두 버전(7400/7500)이 서로 다른 오프셋 필드 크기(4바이트/8바이트)를
  ...
  확인.
- `extract_geometry()`는 정점·폴리곤 **개수**뿐 아니라 실제 정점 좌표
  배열(`vertices`)과 `~i` 디코딩이 끝난 폴리곤별 정점 인덱스 리스트
  (`polygons`)를 그대로 반환한다 — 위 표의 "정확히 일치" 결과는
  `python3 spike/fbx/spike_parser.py <fbx경로>`를 그대로 실행하면
  좌표·폴리곤 값까지 출력되어 재현할 수 있다(최초 버전은 개수만
  반환·출력해 이 재현이 불가능했음 — 코드 리뷰로 보완).
