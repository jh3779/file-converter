...
HWPX_NESTED_SHAPE_SAMPLE = (
    REPO / "spike" / "hwpxlib" / "repo" / "testFile" / "tool" / "textextractor" / "RectInRect.hwpx"
)
...
def _make_tab_hwpx(out_path: Path):
    ...
    proc = subprocess.run([java, "-cp", cp, "MakeTabHwpx", str(out_path)], capture_output=True, text=True, timeout=30)
...
def _run_pagebreakdebug_hwpx(hwpx_path: Path):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        proc = subprocess.run(
            [java, "-cp", cp, "PageBreakDebugHwpx", str(hwpx_path), str(out_path)],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
class TestHwpxWrite(Base):
    ...
    def test_json_to_hwpx_accepts_plain_string_table_cells(self):
        ...
        blocks_json.write_text(
            json.dumps({"blocks": [{"type": "table", "rows": [["a", "b"], ["c", "d"]]}]}), encoding="utf-8"
        )
